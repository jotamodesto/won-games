...
async function getGameInfo(slug) {
  try {
    ...
    const { JSDOM } = jsdom
    const body = await axios.get(`https://www.gog.com/pt/game/${slug}`)
...
async function createManyToManyData(products) {
  ...
  products.forEach(product => {
    ...
    genres && genres.forEach(categories.add, categories)
    supportedOperatingSystems && supportedOperatingSystems.forEach(platforms.add, platforms)
    developers.add(developer)
    publishers.add(publisher)
  })

  return Promise.all([
     ...Array.from(developers).map(name => create(name, 'developer')),
    ...Array.from(publishers).map(name => create(name, 'publisher')),
    ...Array.from(categories).map(name => create(name, 'category')),
    ...Array.from(platforms).map(name => create(name, 'platform'))
  ])
}
...
async function createGames(products) {
  await Promise.all(
    products.map(async product => {
      const existingGame = await getByName(product.title, 'game')
...
module.exports = {
   async populate(params) {
    try {
      const gogApiUrl = `https://www.gog.com/games/ajax/filtered`

      const { data } = await axios.get(gogApiUrl, { params });
      const { products } = data;
